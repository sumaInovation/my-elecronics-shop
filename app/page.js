...
import Link from "next/link"; // Link component එක අමතක කරන්න එපා
    ...
    <div className="flex flex-col bg-white overflow-x-hidden">
        ...
        <div className="mx-auto max-w-7xl px-6 grid md:grid-cols-2 gap-16 items-center">
          <div className="z-10 space-y-8">
            ...
            <div className="flex gap-4">
              ...
              <Link href="/products" className="flex items-center gap-2 bg-slate-900 text-white px-10 py-5 rounded-2xl font-bold hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 active:scale-95">
          ...
          <div className="hidden md:flex justify-end relative">
             ...
             <div className="text-[250px] rotate-12 filter drop-shadow-2xl select-none">💻</div>
          </div>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-7xl px-6 py-24 w-full">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
          ...
          <Link href="/products" className="text-slate-400 font-bold text-sm uppercase tracking-widest hover:text-blue-600 transition-colors">
            View All Products →
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {products.map((p) => (
            <div key={p.id} className="group relative bg-white rounded-[2rem] p-3 transition-all duration-500 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-transparent hover:border-gray-50">
              ...
              {/* Product Image Link */}
              <Link href={`/products/${p.id}`} className="relative aspect-[4/5] bg-[#f9f9f9] rounded-[1.8rem] overflow-hidden block">
              ...
              <div className="px-3 py-6">
                ...
                <Link href={`/product/${p.id}`}>
                  <h3 className="text-lg font-extrabold text-slate-900 hover:text-blue-600 transition-colors cursor-pointer">{p.name}</h3>
                </Link>

                <div className="flex items-center justify-between mt-6">
                  <span className="text-xl font-black text-slate-900 tracking-tighter">{p.price}</span>
                  <button className="bg-slate-50 p-4 rounded-2xl text-slate-900 hover:bg-blue-600 hover:text-white transition-all shadow-sm active:scale-95">
