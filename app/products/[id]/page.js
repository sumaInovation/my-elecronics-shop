...
import { useParams, useRouter } from "next/navigation"; // Router එක import කළා
import { createClient } from "@supabase/supabase-js";
import { ShoppingBag, Star, CheckCircle2, ChevronRight, Tag, Zap } from "lucide-react";
...
export default function ProductDetail() {
  const { id } = useParams();
  const router = useRouter(); // Redirect කරන්න ඕන නිසා
  ...
  // සාමාන්‍ය Add to Cart
  const handleAddToCart = () => {
    addToCart(product);
    alert(`${product.Name} added to cart!`);
  };

  // Buy Now - Add කරලා කෙලින්ම Cart එකට යනවා
  const handleBuyNow = () => {
    addToCart(product);
    router.push("/cart");
  };
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      <div className="mx-auto max-w-7xl px-4 md:px-6 py-8 md:py-12">
        ...
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6 md:mb-10 overflow-x-auto whitespace-nowrap">
          <Link href="/products" className="hover:text-blue-600">Store</Link>
          <ChevronRight size={10} />
          <span className="text-blue-600 uppercase italic">{product.category}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-start">
          ...
          <div className="space-y-6 md:space-y-10 px-2 md:px-0">
            <div className="space-y-4 text-center md:text-left">
              ...
              <div className="flex flex-col md:flex-row items-center gap-4 justify-center md:justify-start">
                <div className="flex items-baseline gap-3">
                  <p className="text-3xl md:text-4xl font-black text-blue-600 tracking-tighter">LKR {product.Price?.toLocaleString()}</p>
                ...
                <div className="flex items-center gap-1 bg-yellow-50 px-3 py-1 rounded-full border border-yellow-100">
                   ...
                   <span className="text-sm font-black text-yellow-700">{product.rating || "5.0"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
               {["Industrial Grade", "1-Year Warranty"].map((item) => (
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <button 
                ...
                className="flex-1 bg-blue-600 text-white py-5 rounded-[1.5rem] font-black uppercase text-xs tracking-[0.2em] hover:bg-slate-900 transition-all shadow-xl shadow-blue-100 flex items-center justify-center gap-2 active:scale-95"
              >
              ...
              <button 
                ...
                className="flex-1 bg-slate-900 text-white py-5 rounded-[1.5rem] font-black uppercase text-xs tracking-[0.2em] hover:bg-blue-600 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2"
              >
           ...
           <div className="flex justify-center md:justify-start gap-8 md:gap-12 border-b border-slate-100 mb-10 overflow-x-auto no-scrollbar">
              {["specs", "reviews"].map((tab) => (
                <button 
                  ...
                  className={`pb-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap ${activeTab === tab ? "border-b-4 border-blue-600 text-blue-600" : "text-slate-400 hover:text-slate-900"}`}
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-0 md:gap-x-20">
                   {product.specifications && Object.entries(product.specifications).map(([key, value]) => (
                       <div key={key} className="flex justify-between items-center py-4 border-b border-slate-50">
                          ...
                          <span className="text-xs md:text-sm font-black text-slate-900 uppercase tracking-tighter">{value}</span>
                       </div>
                   ))}
                   <div className="flex justify-between items-center py-4 border-b border-slate-50">
                        <span className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">Model SKU</span>
                        <span className="text-xs md:text-sm font-black text-slate-900">{product.sku || "N/A"}</span>
                   </div>
                </div>
              ) : (
        ...
        <div className="mt-20 md:mt-32">
           <h3 className="text-xl md:text-2xl font-black text-slate-900 uppercase italic mb-8 md:mb-12 tracking-tighter text-center md:text-left">Related Tech</h3>
              ...
              {relatedProducts.map((rel) => (
                <Link href={`/products/${rel.id}`} key={rel.id} className="group">
                  ...
                  <div className="mt-4 px-1">
                    <h4 className="font-black text-slate-900 text-xs md:text-sm group-hover:text-blue-600 transition-colors line-clamp-1 uppercase tracking-tighter">{rel.Name}</h4>
                    <p className="text-blue-600 font-black text-[10px] md:text-xs mt-1 italic tracking-widest">LKR {rel.Price?.toLocaleString()}</p>
                  </div>
                </Link>
              ))}
           </div>
